...
function PageHero() {
  ...
  const { cms } = useCms();
  const blogPage = (cms.pages || []).find(p => p.key === "blog") || {};
  const cmsTitle = blogPage.heroTitle || "";
  const cmsText = blogPage.heroSubtitle || "";
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-8" style={{ paddingTop: "64px", paddingBottom: "64px" }}>
        ...
        <h1 style={{ margin: "0 0 16px 0", fontFamily: "Arial Black, Arial, sans-serif", fontWeight: 900, fontSize: "clamp(28px, 4vw, 56px)", lineHeight: 1.05, letterSpacing: "-0.5px", textTransform: "uppercase" }}>
          {cmsTitle ? (
            ...
            <>
              <span style={{ color: "var(--fw-website-primary)" }}>{t("blogPage.heroTitle", "KENNIS & ")}</span>
              <span style={{ color: "#fff" }}>{t("blogPage.heroAccent", "INZICHTEN")}</span>
            </>
          )}
        </h1>
        <p style={{ margin: 0, color: "#bbb", fontSize: "clamp(14px, 1.6vw, 17px)", lineHeight: 1.6, maxWidth: "520px" }}>
          {cmsText || t("blogPage.heroText", "Vakartikelen, projectverhalen en technische inzichten vanuit de dagelijkse praktijk van FerroWorks.")}
        </p>
